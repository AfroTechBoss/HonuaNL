...
// =============== Desktop Marketplace ===============
window.DesktopMarketplace = function DesktopMarketplace({ onNav }) {
  const app = useApp();
  return (
          ...
          {MOCK.products.map((p, i) => {
            ...
            <div key={i} className="post-card" onClick={() => app.openModal?.('product', p)} style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--line)', overflow: 'hidden', cursor: 'pointer' }}>
              <div style={{ position: 'relative' }}>
                <ImagePlaceholder label={p.img} height={200} />
...
window.MarketFilters = function MarketFilters() {
...
// =============== Desktop Messages ===============
window.DesktopMessages = function DesktopMessages({ onNav }) {
  const app = useApp();
...
window.DayLabel = function DayLabel({ l }) {
  return (
...
window.Msg = function Msg({ from, children, attach }) {
  const me = from === 'me';
  return (
...
window.DesktopNotifications = function DesktopNotifications({ onNav }) {
  const app = useApp();
  const items = [
...
window.NotifTabs = function NotifTabs() {
...
window.NotifPrefs = function NotifPrefs() {
...
window.Toggle = function Toggle({ on }) {
  return (
    <span style={{
...
// ====================================================================
window.QtyStepper = function QtyStepper({ value, set, min = 1, max = 99, suffix }) {
  return (
...
window.ModalFoot = function ModalFoot({ children }) {
...
window.MProject = function MProject({ data, close }) {
  const app = useApp();
...
window.MArticle = function MArticle({ data, close }) {
  const app = useApp();
...
window.MProduct = function MProduct({ data, close }) {
  const app = useApp();
  ...
  return (
    <Modal onClose={close} width={720}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
        <ImagePlaceholder label={data.img} height={400} />
...
window.MCredit = function MCredit({ data, close }) {
  const app = useApp();
  const [qty, setQty] = React.useState(1);
...
// --- Challenge detail ---
window.MChallenge = function MChallenge({ data, close }) {
  const app = useApp();
...
window.MDiscussion = function MDiscussion({ data, close }) {
  const app = useApp();
  ...
  return (
    <Modal onClose={close} width={620}>
      ...
      <div style={{ display: 'flex', gap: 10, padding: '16px 24px 22px', alignItems: 'center' }}>
        ...
        <button className="btn btn-primary" onClick={() => { const el = document.getElementById('disc-reply'); if (el) el.value = ''; app.toast({ msg: 'Reply posted', kind: 'success', icon: 'comment' }); }}>Reply</button>
      </div>
    </Modal>
  );
};
...
// --- Badge detail ---
window.MBadge = function MBadge({ data, close }) {
  return (
...
// --- Celebration ---
window.MCelebrate = function MCelebrate({ data, close }) {
  return (
...
// --- Wallet ---
window.MWallet = function MWallet({ close }) {
  const app = useApp();
  return (
...
window.MList = function MList({ data, close }) {
  return (
    <Modal onClose={close} width={520}>
...
// --- Community about ---
window.MCommunityAbout = function MCommunityAbout({ data, close }) {
  return (
    <Modal onClose={close} width={500}>
...
window.DesktopAuth = function DesktopAuth({ onNav }) {
  const app = useApp();
  ...
  const field = { width: '100%', boxSizing: 'border-box', background: 'var(--bg-2)', border: '1px solid var(--line)', borderRadius: 11, padding: '12px 14px', fontSize: 15, fontFamily: 'Geist', color: 'var(--ink)', outline: 'none', marginTop: 6 };
      ...
      <div style={{ flex: '0 0 clamp(380px, 38%, 520px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px clamp(24px,4vw,56px)', overflow: 'auto' }} className="no-scrollbar">
        <div style={{ width: '100%', maxWidth: 360 }}>
          ...
          <p style={{ textAlign: 'center', fontSize: 11.5, color: 'var(--ink-4)', marginTop: 14, lineHeight: 1.5 }}>
            By continuing you agree to our <span onClick={() => window.open('../support/' + encodeURIComponent('Terms & Privacy.html'), '_blank')} style={{ color: 'var(--ink-3)', textDecoration: 'underline', cursor: 'pointer' }}>Terms &amp; privacy</span>.
          </p>
        </div>
      </div>
    </div>
