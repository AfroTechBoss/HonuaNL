/* global React, useApp, Icon, DesktopSidebar, Avatar, ImagePlaceholder, Modal, ModalHead,
   SBadge, SStat, SHead, SSpark, sMoney, S_STATUS, MOCK_SELLER, SELLER_CATEGORIES */
...
window.DesktopSellerDashboard = function DesktopSellerDashboard({ onNav }) {
  const app = useApp();
    ...
    <div>
      {lowStock.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 14, marginBottom: 16,
          background: window.sTint('var(--clay)', 12), border: '1px solid color-mix(in srgb, var(--clay) 25%, transparent)' }}>
...
function DashFld({ label, hint, value, onChange, placeholder, prefix, area, type = 'text' }) {
  return (
