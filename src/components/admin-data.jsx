...
window.RoleChip = function RoleChip({ role }) {
  const c = window.ADMIN_ROLES[role] || 'var(--ink-3)';
  return <span style={{ fontSize: 11, fontWeight: 600, fontFamily: 'Geist Mono', color: c, background: window.sTint(c), padding: '2px 8px', borderRadius: 6 }}>{role}</span>;
};

window.REPORT_REASONS = ['Spam', 'Harassment or bullying', 'Hateful content', 'Misinformation', 'Malicious content', 'Inappropriate content', 'Copyright violation', 'Other'];
