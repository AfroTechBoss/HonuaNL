...
window.DesktopForum = function DesktopForum({ onNav }) {
  const app = useApp();
    ...
    <div style={{ display: 'flex', height: '100%', background: 'var(--bg)' }}>
      ...
      <main style={{ flex: 1, display: 'flex', height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ width: 260, borderRight: '1px solid var(--line)', background: 'var(--surface)', padding: '20px 16px', overflow: 'auto' }} className="no-scrollbar">
          ...
          {MOCK.communities.slice(3).map(c => (
            <button key={c.name} style={{
              ...
              <span style={{ width: 24, height: 24, borderRadius: 6, background: 'var(--bg-2)', color: 'var(--ink-2)', display: 'grid', placeItems: 'center', fontSize: 11, fontWeight: 600 }}>{c.name.charAt(0)}</span>
        ...
        <div style={{ flex: 1, overflow: 'auto', height: '100%' }} className="no-scrollbar">
          {/* Cover */}
          <div style={{ height: 160, background: 'linear-gradient(135deg, #c8e6cf 0%, #ecf5ee 100%)', position: 'relative' }}>
            <svg width="100%" height="100%" viewBox="0 0 800 160" preserveAspectRatio="xMidYMid slice">
              <path d="M0 80 Q200 40 400 70 T800 60 L800 160 0 160Z" fill="rgba(31,111,63,.2)"/>
              <path d="M0 120 Q200 90 400 110 T800 100 L800 160 0 160Z" fill="rgba(31,111,63,.3)"/>
            </svg>
          </div>
...
window.DesktopTasks = function DesktopTasks({ onNav }) {
  const app = useApp();
  return (
    <div style={{ display: 'flex', height: '100%', background: 'var(--bg)' }}>
...
// =============== Desktop Bookmarks ===============
window.DesktopBookmarks = function DesktopBookmarks({ onNav }) {
  const app = useApp();
...
window.BookmarkTabs = function BookmarkTabs() {
...
// =============== Desktop Settings ===============
window.DesktopSettings = function DesktopSettings({ onNav }) {
  const app = useApp();
...
window.SettingsSection = function SettingsSection({ sec, app, onNav }) {
  const head = {
    ...
    <>
      ...
      {sec === 'Impact tracking' && (
        <Section title="Connected sources" sub="The more you connect, the more actions verify automatically.">
          {[['Utility provider', 'Pull energy use to verify savings', false], ['Transit / maps', 'Auto-detect low-carbon trips', true], ['Banking (Plaid)', 'Spot sustainable purchases', false]].map(([t, s, on]) => (
            <div key={t} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderTop: '1px solid var(--line)' }}>
      ...
      {sec === 'Connected apps' && (
        <Section title="Linked services">
          {[['Strava', 'Bike & run trips', true], ['Google Calendar', 'Sync project events', true], ['Slack', 'Community alerts', false]].map(([t, s, on]) => (
            <div key={t} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderTop: '1px solid var(--line)' }}>
              <div><div style={{ fontSize: 14, fontWeight: 500 }}>{t}</div><div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 2 }}>{s}</div></div>
      ...
      {sec === 'Help & support' && (
        <Section title="Get help">
          {[['Help center', 'comment', 'Help Center.html'], ['Contact support', 'msg', 'Contact Support.html'], ['Community guidelines', 'users', 'Community Guidelines.html'], ['Report a problem', 'bolt', 'Report a Problem.html'], ['Terms & privacy', 'lock', 'Terms & Privacy.html']].map(([t, ic, file]) => (
            <button key={t} onClick={() => { window.open('../support/' + encodeURIComponent(file), '_blank'); app.toast({ msg: t, sub: 'Opened in a new tab.', icon: ic }); }} className="row-hover" style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '12px 10px', margin: '0 -10px', border: 'none', borderTop: '1px solid var(--line)', background: 'transparent', cursor: 'pointer', fontSize: 14, color: 'var(--ink-2)', fontFamily: 'Geist', textAlign: 'left' }}>
...
window.ToggleC2 = function ToggleC2({ label, sub, def }) {
...
window.Section = function Section({ title, sub, children }) {
  return (
...
window.Field = function Field({ label, v, multiline }) {
  return (
...
window.ToggleRow = function ToggleRow({ label, sub, on }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0' }}>
      <div>
...
window.MCompose = function MCompose({ close }) {
  const app = useApp();
...
// --- Log an action ---
window.MLogAction = function MLogAction({ close }) {
  const app = useApp();
...
// --- Export report ---
window.MExport = function MExport({ close }) {
  const app = useApp();
...
// --- Start a project ---
window.MStartProject = function MStartProject({ close }) {
  const app = useApp();
...
// --- Create challenge ---
window.MCreateChallenge = function MCreateChallenge({ close }) {
  const app = useApp();
...
// --- New collection ---
window.MNewCollection = function MNewCollection({ close }) {
  const app = useApp();
  const [name, setName] = React.useState('');
  return (
...
// --- Edit profile ---
window.MEditProfile = function MEditProfile({ close }) {
  const app = useApp();
  return (
    <Modal onClose={close} width={560}>
...
// --- Tip a user ---
window.MTip = function MTip({ data, close }) {
  const app = useApp();
...
window.MAutoOffset = function MAutoOffset({ close }) {
  const app = useApp();
  return (
...
// --- Offset my year ---
window.MOffsetYear = function MOffsetYear({ close }) {
  const app = useApp();
  return (
    <Modal onClose={close} width={480}>
...
window.MDeleteAccount = function MDeleteAccount({ close }) {
  const app = useApp();
  return (
    <Modal onClose={close} width={460}>
...
window.MDeleteAccount2 = function MDeleteAccount2({ close }) {
  const app = useApp();
...
// =============== Dispatcher ===============
window.ModalContent = function ModalContent({ type, data, close }) {
  const map = {
