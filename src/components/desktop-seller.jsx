/* global React, useApp, Icon, DesktopSidebar, Avatar, Logo, ImagePlaceholder,
   SStepper, SBadge, SStat, SHead, sMoney, SELLER_CATEGORIES, SELLER_PRACTICES, SELLER_CERTS, MOCK_APPLICATIONS, MOCK_SELLER */

// =====================================================================
// Small form helpers (prefixed to avoid global collisions)
// =====================================================================
function FldText({ label, hint, value, onChange, placeholder, prefix, area, type = 'text' }) {
  return (
...
function ChipGroup({ options, value = [], onToggle, single }) {
  return (
...
function CheckRow({ checked, onToggle, title, sub }) {
  return (
...
// =====================================================================
window.DesktopSell = function DesktopSell({ onNav }) {
  const app = useApp();
  ...
  return (
    <StatusShell>
      <div style={{ textAlign: 'center' }}>
        <span style={{ width: 64, height: 64, borderRadius: 20, background: window.sTint('var(--sun)', 18), color: 'var(--sun)', display: 'inline-grid', placeItems: 'center' }}><Icon name="clock" size={30} stroke={2} /></span>
...
function SellRejected({ onNav }) {
  ...
  return (
    <StatusShell>
      <div style={{ textAlign: 'center' }}>
        <span style={{ width: 64, height: 64, borderRadius: 20, background: window.sTint('var(--clay)', 16), color: 'var(--clay)', display: 'inline-grid', placeItems: 'center' }}><Icon name="close" size={30} stroke={2.4} /></span>
      ...
      <div style={{ marginTop: 26, display: 'grid', gap: 10 }}>
        ...
        <button className="btn btn-ghost" onClick={() => window.open('../support/' + encodeURIComponent('Contact Support.html'), '_blank')} style={{ justifyContent: 'center', padding: '13px' }}>Contact the review team</button>
      </div>
    </StatusShell>
  );
}
