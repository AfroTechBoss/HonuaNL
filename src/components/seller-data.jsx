...
window.sTint = function sTint(color, pct = 14) {
  return `color-mix(in srgb, ${color} ${pct}%, transparent)`;
};

// Status badge -------------------------------------------------------
window.SBadge = function SBadge({ status, dot = true, size = 'md' }) {
  const meta = window.S_STATUS[status] || { label: status, color: 'var(--ink-3)' };
  const pad = size === 'sm' ? '2px 8px 2px 7px' : '4px 11px 4px 9px';
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: 6,
      padding: pad, borderRadius: 999,
      background: window.sTint(meta.color), color: meta.color,
...
// Money formatter ----------------------------------------------------
window.sMoney = function sMoney(n, dp = 2) {
  return '$' + Number(n || 0).toLocaleString('en-US', { minimumFractionDigits: dp, maximumFractionDigits: dp });
};

// Stat card ----------------------------------------------------------
window.SStat = function SStat({ label, value, sub, icon, accent = 'var(--green)', trend }) {
  return (
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {icon && (
          <span style={{ width: 30, height: 30, borderRadius: 9, display: 'grid', placeItems: 'center',
            background: window.sTint(accent), color: accent }}>
...
// Tiny sparkline -----------------------------------------------------
window.SSpark = function SSpark({ data = [], color = 'var(--green)', w = 120, h = 36, fill = true }) {
...
window.SStepper = function SStepper({ steps, current, onJump, compact }) {
  return (
...
window.SHead = function SHead({ kicker, title, children }) {
  return (
...
window.SELLER_CERTS = ['B Corp', 'Fair Trade', 'FSC', 'GOTS', 'Climate Neutral', '1% for the Planet'];
