...
window.MOCK = {
  users: {
    sarah:   { name: 'Sarah Green', handle: 'sarahgreen', score: 92, verified: true, avatar: null },
    marcus:  { name: 'Marcus Johnson', handle: 'ecomarcus', score: 78, verified: false, avatar: null },
    greentech: { name: 'GreenTech Solutions', handle: 'greentech', score: 96, verified: true, avatar: null },
    maya:    { name: 'Maya Patel', handle: 'urbangrower', score: 71, verified: false, avatar: null },
    can:     { name: 'Climate Action Network', handle: 'climateactnow', score: 88, verified: true, avatar: null },
    you:     { name: 'You', handle: 'you', score: 86, verified: false, avatar: null },
    tara:    { name: 'Tara Lin', handle: 'tara_l', score: 64, verified: false, avatar: null },
    okafor:  { name: 'Dr. Adaeze Okafor', handle: 'dr_okafor', score: 94, verified: true, avatar: null },
  },
  posts: [
    {
      id: 1, user: 'sarah',
  ...
  communities: [
    { name: 'Urban gardeners', members: '24.1k', cover: 'urban garden hero', cat: 'Agriculture' },
    { name: 'Solar DIY', members: '18.6k', cover: 'solar panels diy', cat: 'Energy' },
    { name: 'Ocean cleanup crew', members: '41.3k', cover: 'beach cleanup volunteers', cat: 'Ocean' },
    { name: 'Climate policy nerds', members: '12.9k', cover: 'climate policy paper', cat: 'Policy' },
    { name: 'Zero-waste households', members: '33.7k', cover: 'mason jars pantry', cat: 'Waste' },
    { name: 'EV owners', members: '52.0k', cover: 'EV charging', cat: 'Transport' },
  ],
  products: [
    { name: 'Refillable shampoo bar', price: '$12', brand: 'BareHaus', tag: 'Plastic-free', img: 'shampoo bar product shot' },
    { name: 'Solar phone charger', price: '$48', brand: 'Sunly', tag: 'Solar', img: 'solar charger flat lay' },
    { name: 'Bamboo cutlery set', price: '$18', brand: 'Forrest', tag: 'Compostable', img: 'bamboo cutlery roll' },
    { name: 'Reusable produce bags', price: '$16', brand: 'Loop', tag: 'Zero-waste', img: 'mesh produce bags' },
    { name: 'Wool insulation panel', price: '$89', brand: 'Hjem', tag: 'Carbon-neg', img: 'wool insulation roll' },
    { name: 'Compost starter kit', price: '$34', brand: 'Soily', tag: 'Soil', img: 'compost starter kit' },
  ],
  trends: [
...
// =============== Post card ===============
window.PostCard = function PostCard({ post, dense = false }) {
  const app = useApp();
  ...
  return (
    <article onClick={open} className="post-card" style={{
      ...
      <header style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
        <span onClick={stop(() => app.nav?.('profile', { handle: user.handle }))}><Avatar name={user.name} size={42} /></span>
      ...
      {post.image && (
        <div style={{
          ...
          <ImagePlaceholder label={post.image} height={dense ? 200 : 280} />
          {post.image2 && <ImagePlaceholder label={post.image2} height={dense ? 200 : 280} />}
        </div>
      )}
...
window.ActionBtn = function ActionBtn({ icon, count, active, activeColor = 'var(--green)', onClick }) {
  return (
...
window.formatCount = function formatCount(n) {
  if (n >= 1000) return (n / 1000).toFixed(n >= 10000 ? 0 : 1) + 'k';
  return String(n);
};

// Right column trending widget for desktop
window.TrendingPanel = function TrendingPanel() {
  const app = useApp();
  return (
    <div style={{
      background: 'var(--surface)', borderRadius: 16,
...
window.MyImpactCard = function MyImpactCard() {
  const app = useApp();
  return (
...
// Suggested follows
window.SuggestedFollows = function SuggestedFollows() {
  const app = useApp();
      ...
      {list.map(k => {
        ...
        return (
          <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0' }}>
            <span onClick={() => app.nav?.('profile', { handle: u.handle })} style={{ cursor: 'pointer' }}><Avatar name={u.name} size={36} /></span>
...
let __cid = 9000;
window.makeCommentSeed = function makeCommentSeed() {
  return [
...
window.CommentThread = function CommentThread({ tree, setTree, dense = false }) {
...
window.CommentNode = function CommentNode({ node, depth, like, reply, dense, defaultOpen }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 10, paddingTop: 14 }}>
      <Avatar name={usr.name} size={av} verified={usr.verified} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
