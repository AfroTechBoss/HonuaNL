...
// =============== Desktop Profile ===============
window.DesktopProfile = function DesktopProfile({ onNav, params }) {
  ...
  const u = isOwn
    ? { name: 'You', handle: 'you', score: 86, verified: false }
      ...
      <main style={{ flex: 1, overflow: 'auto', height: '100%' }} className="no-scrollbar">
        ...
        <div style={{ padding: '0 32px', maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{
            ...
            <div style={{
              width: 132, height: 132, borderRadius: 24, background: 'var(--green)',
              border: '6px solid var(--bg)', color: '#fff',
              display: 'grid', placeItems: 'center', fontSize: 56, fontFamily: 'Bricolage Grotesque',
              fontWeight: 600,
            }}>{u.name.charAt(0)}</div>
            ...
            <div style={{ display: 'flex', gap: 8, paddingBottom: 8 }}>
              {isOwn ? (
                ...
                <>
                  ...
                  <button className="btn btn-ghost" onClick={() => app.openModal('tip', { user: u })} title="Tip"><Icon name="gift" size={14} /> Tip</button>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.8fr 1fr', gap: 18, marginBottom: 18 }}>
            <div style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--line)', padding: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 20, marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--line)' }}>
                ...
                <Stat n="48.6k" l="Followers" />
                <Stat n="412" l="Following" />
                ...
                {[
                  ...
                  <div key={i} style={{ padding: '10px 0', borderTop: i === 0 ? 'none' : '1px solid var(--line)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 500 }}>
                      <span>{t}</span>
                      <span style={{ fontFamily: 'Geist Mono', color: 'var(--ink-3)', fontSize: 11 }}>{Math.round(p * 100)}%</span>
                    </div>
                    ...
                    <div style={{ height: 4, background: 'var(--line)', borderRadius: 999, marginTop: 6 }}>
                      <div style={{ width: (p * 100) + '%', height: '100%', background: 'var(--green)', borderRadius: 999 }} />
                    </div>
                  </div>
                ))}
              </div>
...
window.CommentRow = function CommentRow({ usr, text, time, likes, onNav }) {
  const [liked, setLiked] = React.useState(false);
  return (
    <div style={{ display: 'flex', gap: 12, padding: '14px 0', borderTop: '1px solid var(--line)' }}>
      <span onClick={() => onNav?.('profile', { handle: usr.handle })} style={{ cursor: 'pointer' }}><Avatar name={usr.name} size={36} /></span>
      <div style={{ flex: 1 }}>
...
window.Stat = function Stat({ n, l, green, light }) {
  return (
    <div>
      <div style={{
...
// =============== Desktop Post Detail ===============
window.DesktopPostDetail = function DesktopPostDetail({ onNav, params }) {
  const app = useApp();
        ...
        <div style={{
          ...
          <article style={{ background: 'var(--surface)', borderRadius: 20, border: '1px solid var(--line)', padding: 28, marginBottom: 16 }}>
            <header style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
              <span onClick={() => onNav?.('profile', { handle: user.handle })} style={{ cursor: 'pointer' }}><Avatar name={user.name} size={56} /></span>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {post.image && <ImagePlaceholder label={post.image} height={420} />}
            <div style={{
